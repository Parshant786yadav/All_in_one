...
# Load .env from Backend directory (app may be run from project root)
...
async def upload_document(
    ...
    try:
        ...
        for chunk in chunks:
            ...
            db_ops.insert_document_chunk(document["id"], chunk, json.dumps(embedding))
...
config = Config(env_file=os.path.join(_BACKEND_DIR, ".env"))
...
def _get_mount_prefix(request: Request) -> str:
    """Prefix when app is mounted (e.g. /llm-chatbot). Set env LLM_CHATBOT_MOUNT_PATH if root_path is not set."""
    root_path = (request.scope.get("root_path") or "").strip().rstrip("/")
    if root_path:
        return root_path
    return (os.getenv("LLM_CHATBOT_MOUNT_PATH") or "").strip().rstrip("/")


def _get_origin(request: Request) -> str:
    """Scheme + host (e.g. http://127.0.0.1:8000) from request scope."""
    scheme = request.scope.get("scheme", "http")
    server = request.scope.get("server") or ("localhost", 8000)
    host = server[0] if isinstance(server, (list, tuple)) else server
    port = server[1] if isinstance(server, (list, tuple)) and len(server) > 1 else (8000 if scheme == "http" else 443)
...
async def login_google(request: Request):
    if not os.getenv("GOOGLE_CLIENT_ID") or not os.getenv("GOOGLE_CLIENT_SECRET"):
        return RedirectResponse(
            _get_origin(request) + _get_mount_prefix(request) + "/?error=oauth&message="
            + urllib.parse.quote("GOOGLE_CLIENT_ID and GOOGLE_CLIENT_SECRET must be set in .env")
        )
    try:
        prefix = _get_mount_prefix(request)
        origin = _get_origin(request)
        path = request.url_for("auth_google")  # e.g. "/auth/google"
        redirect_uri = origin.rstrip("/") + prefix + path
        return await oauth.google.authorize_redirect(request, redirect_uri)
    except Exception as e:
        # Avoid 500; send user back to app with error (log in production)
        frontend_url = _get_origin(request) + _get_mount_prefix(request)
        msg = urllib.parse.quote(str(e)[:200])
        return RedirectResponse(frontend_url + "/?error=oauth&message=" + msg)

@app.get("/auth/google")
async def auth_google(request: Request):
    prefix = _get_mount_prefix(request)
    base = str(request.base_url).rstrip("/")
    # After login, send user to the frontend (when mounted, use base + prefix e.g. http://localhost:8000/llm-chatbot)
    frontend_url = os.getenv("FRONTEND_URL", "").strip() or (base + prefix)
    frontend_url = frontend_url.rstrip("/")
    try:
        token = await oauth.google.authorize_access_token(request)
    except OAuthError as e:
        # Show the exact redirect URI they must add in Google Cloud Console (must include mount path)
        correct_redirect = base + prefix + "/auth/google"
        error_msg = urllib.parse.quote(
            "Google sign-in failed. In Google Cloud Console, add this exact Authorized redirect URI: " + correct_redirect
        )
